...
pub fn get_problem_dir(app: &App, problem: &str) -> Result<PathBuf, Report> {
...
pub fn get_test_dir(app: &App, problem: &str) -> Result<PathBuf, Report> {
    ...
    if !test_dir.exists() {
        return Err(eyre::eyre!("👀 Looks like you already have tests for {problem}"));
    } else {
        Ok(test_dir)
    }
}
...
pub fn copy_template(app: &App, args: &Get, problem: &str) -> Result<(), Report> {
    ...
    // find the language for the problem by first checking the command line args
    // if none is found, use the default language from the config
    // if command line arg is found, check if it is a valid language i.e., there is an entry for it in the config
    // the
    // if it is not a valid language, throw an error
    // if it is a valid language, use it
    ...
    // if it does exist, copy it to the problem directory and rename it to the problem id
    
    if let Some(template_path) = template_path {
        if !Path::new(&template_path).exists() {
            eyre::bail!("🙀 Template file does not exist: {}", template_path.display());
        }
